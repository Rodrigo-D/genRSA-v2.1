...
/*
 * La forma de recuperar la clave es distinta según esta clave sea pública o privada, por ello la
 *  existencia de dos métodos loadPrivateKey() y loadPublicKey(). La diferencia es que se debe usar
 *   una clase hija de EncodedKeySpec para obtener la clave a partir de los bytes leídos en fichero 
 *   y no vale la misma clase hija. Hay varias posibilidades, pero para clave pública podemos usar
	X509EncodedKeySpec y para la clave privada podemos usar PKCS8EncodedKeySpec, ambas hijas de EncodedKeySpec.
 Una vez que tenemos estas instancias y la de keyFactory, podemos usar el método keyFactory.generatePrivate
